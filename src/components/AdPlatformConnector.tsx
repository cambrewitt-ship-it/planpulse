...
export default function AdPlatformConnector({ clientId }: AdPlatformConnectorProps) {
  ...
  // Fetch accounts when connection status changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (openModal === 'google-ads' && connectionStatus['google-ads']) {
      fetchGoogleAdsAccounts();
    }
  ...
  const PlatformLogo = ({ platformId }: { platformId: string }) => {
    ...
    } else if (platformId === 'google-analytics') {
      return (
        <svg className="w-12 h-12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    return <div className="text-4xl">{platforms.find(p => p.id === platformId)?.icon}</div>;
  };
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-3">
                ...
                <span>{platforms.find(p => p.id === openModal)?.displayName}</span>
              ...
              <DialogDescription>
                {platforms.find(p => p.id === openModal)?.description}
              </DialogDescription>
            </DialogHeader>
              ...
              {!connectionStatus[openModal] ? (
                <div className="space-y-4">
                  ...
                  <Button
                    ...
                    className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-medium py-2 px-4 rounded-lg"
                  >
                ...
                <div className="space-y-4">
                  <div className="flex gap-3">
                    <Button
                      variant="outline"
                      onClick={() => handleDisconnect(openModal)}
                      disabled={loadingStates[openModal]}
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                        ...
                        {discoveredMetaAccounts.length > 0 && (
                          <div className="space-y-2">
                            ...
                            >
                              {isSavingMetaAccounts ? 'Saving...' : `✓ Save Selected Accounts (${selectedMetaAccounts.size})`}
                            </Button>
                          </div>
                        )}
                      </div>
                        ...
                        {discoveredGoogleAnalyticsAccounts.length > 0 && (
                          <div className="space-y-2">
                            ...
                            <Button
                              ...
                              className="w-full bg-green-600 hover:bg-green-700 text-white"
                            >
                              {isSavingGoogleAnalyticsAccounts ? 'Saving...' : `✓ Save Selected Properties (${selectedGoogleAnalyticsAccounts.size})`}
                            </Button>
                          </div>
                        )}
                      </div>
