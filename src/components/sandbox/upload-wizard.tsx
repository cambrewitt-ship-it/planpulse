    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-8">
        <div className="w-full max-w-2xl">
          ...
          {/* Fees detected — pulled out of the channel grid into the fees strip */}
          {parsedPlan.fees && parsedPlan.fees.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4 flex items-center gap-2 flex-wrap">
              <span className="text-xs font-semibold text-amber-700 uppercase tracking-wide flex-shrink-0">Fees detected (added as non-media fees):</span>
              {parsedPlan.fees.map(fee => (
                <span key={fee.id} className="text-xs bg-amber-100 text-amber-800 rounded px-2 py-0.5 font-medium">
                  {fee.name}{fee.amount > 0 ? ` — ${formatBudget(fee.amount)}` : ''}
                </span>
              ))}
            </div>
          )}
