...
export default function ClientFunnelsPage() {
  ...
  useEffect(() => {
    console.log('[Funnels] useEffect triggered, clientId:', clientId);
    if (clientId) {
  ...
  useEffect(() => {
    ...
    if (selectedChannelFilter === 'all') {
      ...
      setFilteredFunnels(allFunnels.filter(f => f.channelIds.includes(selectedChannelFilter)));
    }
  }, [selectedChannelFilter, allFunnels]);

  const loadChannelsAndFunnels = async () => {
    console.log('[Funnels] loadChannelsAndFunnels called with clientId:', clientId);
    setLoading(true);
    try {
      // Load all media channels for this client
      const url = `/api/media-plan/channels?clientId=${clientId}`;
      console.log('[Funnels] Fetching channels from:', url);
      const channelsResponse = await fetch(url);
      console.log('[Funnels] Channels response status:', channelsResponse.status);
      const channelsData = await channelsResponse.json();
      
      console.log('[Funnels] Channels API response:', channelsData);
      
      if (channelsData.success && channelsData.channels) {
        console.log('[Funnels] Setting channels:', channelsData.channels);
        setChannels(channelsData.channels);

        // Load all funnels for this client
        const funnelsResponse = await fetch(`/api/funnels?clientId=${clientId}`);
        const funnelsData = await funnelsResponse.json();

        if (funnelsData.success && funnelsData.funnels) {
          setAllFunnels(funnelsData.funnels);
          setFilteredFunnels(funnelsData.funnels);

          // Auto-select first funnel if available
          if (funnelsData.funnels.length > 0 && !activeFunnelId) {
            setActiveFunnelId(funnelsData.funnels[0].id);
            await calculateFunnel(funnelsData.funnels[0].id);
          }
        } else {
          setAllFunnels([]);
          setFilteredFunnels([]);
      ...
      console.error('[Funnels] Failed to load channels and funnels:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getChannelNames = (channelIds: string[]) => {
    if (!channelIds || channelIds.length === 0) return 'No channels';
    if (channelIds.length === channels.length) return 'All Channels';
    
    const names = channelIds
      .map(id => channels.find(c => c.id === id)?.name)
      .filter(Boolean);
    
    return names.length > 0 ? names.join(', ') : 'Unknown Channels';
  };

  const getChannelPlatforms = (channelIds: string[]) => {
    if (!channelIds || channelIds.length === 0) return [];
    
    const platforms = channelIds
      .map(id => channels.find(c => c.id === id)?.platform)
      .filter(Boolean)
      .filter((platform, index, self) => self.indexOf(platform) === index); // unique
  ...
  return (
    <div className="container mx-auto p-6">
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {getChannelNames(funnel.channelIds)}
                            </p>
                            <div className="flex flex-wrap gap-1 mt-2">
                              {getChannelPlatforms(funnel.channelIds).map((platform) => (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {getChannelNames(calculatedFunnel.config.channelIds)}
                    </CardDescription>
                  </div>
      ...
      {isBuilderOpen && console.log('[Funnels] Opening modal with channels:', channels)}
      <FunnelBuilderModal
