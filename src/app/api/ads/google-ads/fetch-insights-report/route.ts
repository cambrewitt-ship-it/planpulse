...
import { syncGoogleAdsInsightsReport } from '@/lib/ads/google-ads-insights-report';

/**
 * On-demand sync for the Client Hub "Google Ads — Insights" section (search
 * terms, region breakdown, device breakdown). Triggered by the section's
 * "Sync insights data" button — never the always-on 6h cron. Mirrors the
 * auth/connection-resolution shape of
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await syncGoogleAdsInsightsReport({
