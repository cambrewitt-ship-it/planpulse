...
/** Public, token-gated Dimension Explorer read — same trust model as api/hub/[token]/route.ts. */
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (hiddenCards.includes('dimensionExplorer')) {
    return NextResponse.json({ error: 'Card not visible' }, { status: 403 });
  }
  ...
  const rows = await getGA4DimensionExplorer(admin, link.client_id, dimension, { start, end });
